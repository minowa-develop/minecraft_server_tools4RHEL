...
- common.conf
  ...
  JAVA_PATH: Java版のみ使用します、Javaのパスを指定してください(※1)
  USE_SERVER: 使用するサーバ名
  BACKUP_PATH: バックアップ先のパス
  ZIP7_HOME: 7zipのパス,記載していればバックアップ時に使用し、記載無しの場合はzipで圧縮する
  BEDROCK_SERVER_URL: BE版のサーバモジュールをダウンロードするURL(※2)
  JAVA_SERVER_URL: Java版のサーバモジュールをダウンロードするURL(※2)
  
  ※1 javaは下記にてダウンロードしてください(Javaバージョンについては各minecraftのバージョンに合うものを使用してください)
    https://adoptium.net/temurin/releases/?os=linux&arch=x64&package=jre&version=11 
  ※2 バージョンアップの際に変更する必要があります。サーバモジュールの最新は下記のURLで適宜変更してください
